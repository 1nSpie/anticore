    ...
    return products.map((product) => (
      <div key={product.id} className="group">
        ...
        <div className="p-0.5 border-orange border-s-2">
          ...
          <span className="text-gray-200">{product.carlist}</span>
        </div>

        <div className="mt-4 space-y-2">
          <div className="flex items-center gap-2">
            <span className="font-bold text-white dark:text-orange">
              Днище:
            </span>
            <span className="text-white">{product.bottom}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-800 dark:text-orange">
              Compex:
            </span>
            <span className="text-white">{product.compex}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-800 dark:text-orange">
              Compex + Diamond:
            </span>
            <span className="text-white">{product.compexAndDiamond}</span>
          </div>
        </div>
      </div>
    ));
